...
export class WeatherService {
  ...
  constructor(public http: HttpClient) {
   ...
   getCityWeatherByName(city:string, metric:'metric' | 'imperial' = 'metric'): Subject<string>{
     ...
     this.http.get(
       `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=${metric}&APPID=81bc22e9cdc6cb9918e9368b2840fd8c`)
       .subscribe((data)=>{
   ...
   getCitiesWeathersByNames(cities:Array<string>, metric: 'metric' | 'imperial' = 'metric'): Subject<any>{
     ...
     cities.forEach((city)=>{
       citiesSubject.next(this.http.get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=${metric}&APPID=81bc22e9cdc6cb9918e9368b2840fd8c`));
   ...
   getWeatherState(city:string):Subject<string>{
     ...
     this.http.get(
       `https://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=81bc22e9cdc6cb9918e9368b2840fd8c`)
       .subscribe((data) => {
   ...
   getCurrentWind(city:string, metric:'metric' | 'imperial' = 'metric'): Subject<number>{
    ...
    getMaxTemp(city:string, metric: 'metric' | 'imperial' = 'metric'): Subject<number> {
      ...
      let max : number;
      this.http.get(
        `https://api.openweathermap.org/data/2.5/weather?q=${city}&unit=${metric}&APPID=81bc22e9cdc6cb9918e9368b2840fd8c`
      ).subscribe((weather:any)=>{
